...
/// </summary>
/// COMPLEXITY: Advanced
/// CONCEPTS: Render textures, Post-processing, Custom inspectors
...
public class SRSRenderSettings : ScriptableObject {
	...
	private class SpaceRailShooterSettingsInspector : Editor {
		...
		private const string DIALOG_MESSAGE = "This will adjust project quality settings for the entire project, and post-processing settings if a PP file is selected.";
		...
		public override void OnInspectorGUI() {
			...
			if (settings.useCustomSettings) {
				...
				//post-process and render texture
#if POST_PROCESSING
				settings.postProcessingProfile = (PostProcessingProfile)EditorGUILayout.ObjectField("Post-processing file", settings.postProcessingProfile, typeof(PostProcessingProfile), false);
#endif
				...
				if (settings.postProcessingProfile != null) {
					EditorGUILayout.LabelField("PP anti-aliasing enabled", settings.postProcessingProfile.antialiasing.enabled.ToString());
				}
#endif
				...
				if (GUILayout.Button("Apply 16-bit preset")) {
					if (EditorUtility.DisplayDialog(DIALOG_TITLE, DIALOG_MESSAGE, DIALOG_OK, DIALOG_CANCEL)) {
						...
						setQuality(0);
#if POST_PROCESSING
						if (settings.postProcessingProfile != null) settings.postProcessingProfile.antialiasing.enabled = false;
#endif
					}
				}
				...
				if (GUILayout.Button("Apply 64-bit preset")) {
					if (EditorUtility.DisplayDialog(DIALOG_TITLE, DIALOG_MESSAGE, DIALOG_OK, DIALOG_CANCEL)) {
						...
						setQuality(2);
#if POST_PROCESSING
						if (settings.postProcessingProfile != null) settings.postProcessingProfile.antialiasing.enabled = true;
#endif
					}
				}
